...
productionFilePath <- file.path(dir$data, "production", "PVS Fig 1 2022.xlsx")
...
pvProduction <- read_excel(productionFilePath, sheet = "Data") %>% 
    rename(country = `...1`) %>% 
    filter(!is.na(country)) %>% 
    filter(country != "Total") %>% 
    mutate(
        country = str_to_lower(country), 
        country = ifelse(country == 'pr china', 'china', country),
        country = ifelse(country == 'united states', 'us', country),
        country = ifelse(country == 'rest of world', 'row', country)
    ) %>%
    pivot_longer(
        names_to = 'year', 
        values_to = 'production_gw',
        cols = `2010`:`2021`
    )
