...
cost_historical_plot <- cost_all %>%
    dplyr::filter(component == "Module") %>%
    select(
        year, country, cost_per_kw, cost_per_kw_lb, cost_per_kw_ub, 
        scenario) %>% 
    mutate(
        scenario = fct_relevel(scenario, c("S1", "BAU")),
        scenario = fct_recode(scenario,
            "Global learning" = "BAU",
            "National learning" = "S1"),
        year = lubridate::ymd(paste0(year, "-01-01"))) %>% 
    ggplot() +
    # First add historical dashed line
    geom_line(
    ...
    geom_line(
        aes(x = year, y = cost_per_kw, color = scenario, linetype = scenario,
            alpha = scenario),
        size = 1) +
    ...
    scale_linetype_manual("Scenario", values = c(1, 1)) +
    scale_alpha_manual("Scenario", values = c(0.6, 0.6)) +
    theme_minimal_grid(
        font_size = 16,
    ...
    geom_segment(
        data = data.frame(
            ...
            y = c(500, 500, 500), 
            yend = c(700, 850, 800), 
            country = c("China", "Germany", "USA")),
        aes(x = x, y = y, xend = xend, yend = yend), 
        color = "grey60", size = 0.5
    )

# colorblindr::cvd_grid(cost_historical)
