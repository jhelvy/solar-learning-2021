...
compareCapData <- function(df) {
    ...
    result <- result %>% 
        mutate(label = paste0("λ = ", lambda, ", p_i = ", median_p))
    return(result)
}
```
...
The U.S. and Germany serve as an illustrative comparison. Although Germany has a large domestic capacity, it's domestic _production_ is relatively low. For Germany, a relatively large value for $\lambda_i$ fits the data better, suggesting that Germany's national installed capacity has contributed more to it's learning, but that goes against the knowledge that Germany has very limited domestic production. In contrast, a smaller value for $\lambda_i$ fits the U.S. data better, suggesting less learning from national capacity, which is more consistent with the U.S.'s limited PV production. 

So for two countries with sizeable domestic capacities and limited domestic production, why would the same model suggest one has significantly greater domestic learning?
